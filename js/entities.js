...
export class Missile {
    ...
    update(width, height) {
        ...
        // Out of bounds
        if (this.x < -40 || this.x > width + 40 || this.y < -60 || this.y > height + 60) {
            this.active = false;
        }
    }
    ...
    draw(ctx) {
        ...
        for (let i = 0; i < this.trail.length; i++) {
            ...
            ctx.save();
            ctx.beginPath();
            ctx.arc(t.x, t.y, 5 * (i / this.trail.length) + 2.5, 0, Math.PI * 2);
            ctx.fillStyle = i % 2 === 0 ? `rgba(255, 120, 0, ${t.alpha * 0.8})` : `rgba(255, 230, 40, ${t.alpha * 0.9})`;
            ctx.fill();
            ctx.restore();
        }
